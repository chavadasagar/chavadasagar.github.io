...
(function () {
  ...
  const BoilerplateCleaner = {
    ...
    cleanText(text) {
      ...
      res = res.replace(/^[ \t]*(?:\?|❮|❯)?\s*(?:Home|Previous|Next)\s*(?:\?|❮|❯)?\s*(?:(?:Home|Previous|Next)\s*(?:\?|❮|❯)?)*[ \t]*$/gim, '');
      res = res.replace(/ny:-->/gi, '');
      res = res.replace(/<!--[\s\S]*?-->/gi, '');
      res = res.replace(/<!--|-->/gi, '');
      res = res.replace(/^[ \t]*(?:Try it Yourself|Try it yourself|TRY IT YOURSELF)[ \t]*[»❯›]?[ \t]*$/gim, '');
      res = res.replace(/^[ \t]*[»❯❮★■▲▼•]+[ \t]*$/gim, '');
      res = res.replace(/^[ \t]*\+1\s+Sign in to track progress[ \t]*$/gim, '');
      res = res.replace(/^[ \t]*REMOVE ADS[ \t]*$/gim, '');
      return res.trim();
    },
  ...
  function isExampleHeader(str) {
    if (!str) return false;
    const s = str.trim();
    if (/^Example\s+Explained$/i.test(s)) return 'explained';
    if (/^(?:[A-Za-z0-9+#_.\- ]+\s+)?(?:Example|Examples|Syntax|Code\s*Snippet)(?:\s*[:\-0-9]|\s*\([^\)]+\))?$/i.test(s)) {
      if (s.split(' ').length <= 6 && !/[.!?,;]$/.test(s.replace(/[:\-]$/, ''))) {
        return 'example';
      }
    }
    return false;
  ...
  function isDefiniteCodeLine(str) {
    if (!str) return false;
    const s = str.trim();
    if (/^<!DOCTYPE/i.test(s)) return true;
    if (/^<\/?(html|head|title|body|div|p|span|script|style|link|meta|table|tr|td|th|thead|tbody|tfoot|ul|ol|li|form|input|button|label|select|option|textarea|svg|canvas|section|article|header|footer|nav|aside|main|h[1-6]|pre|code|img|a|b|i|strong|em|hr|br)[\s>/]/i.test(s)) return true;
    if (/^[a-zA-Z0-9_\-\.#:,\s>+~*]+\s*\{\s*$/.test(s) && !s.includes('http') && s.length < 80) return true;
    if (/^[a-zA-Z\-]+\s*:\s*[^;]+;\s*$/.test(s)) return true;
    if (s === '{' || s === '}' || s === '};' || s === '});' || s === '<?php' || s === '?>') return true;
    if (/^(const|let|var|function|def|class|import|from|export|#include|package|namespace|using|public|private|protected|static|void|int|float|double|char|bool|SELECT|FROM|WHERE|CREATE|INSERT|UPDATE|DELETE)\b/i.test(s)) return true;
    if (/^(console\.log|document\.getElementById|document\.querySelector|print|System\.out\.println|printf|echo|cin|cout)\b/.test(s)) return true;
    if (/^(hsla?|rgba?)\([^\)]+\)/i.test(s)) return true;
    if (/^\/\*[\s\S]*\*\/$/.test(s) || /^\/\/[^\n]*$/.test(s) || /^#[^\n]*$/.test(s)) return true;
    return false;
  }

  function isCssSelectorBeforeBrace(lines, idx) {
    const line = (lines[idx] || '').trim();
    if (!line || line.length > 50 || /[.;!?]$/.test(line)) return false;
  ...
  function isProseLine(str) {
    if (!str) return false;
    const s = str.trim();
    if (isDefiniteCodeLine(s)) return false;
    
    if (/^(However|Note|Important|Tip|Warning|In the example|As you can see|The|This|These|That|You|When|If|Remember|Because|Notice|Output|Result|Click|Press|Here|To|For|Since|Some|All|Every|Each|An|A|With|We|By|From|At|On|Assume|Then|Now|Below|Above|In this|In our|To see|First|Second|Finally|Next|Also|Such|In addition)\b/i.test(s)) {
      return true;
    }

    const cleanNoTags = s.replace(/<[^>]+>/g, '').trim();
    if (/^[A-Z][a-z]+ [a-z]+ [a-z]+/.test(cleanNoTags) && !cleanNoTags.includes('{') && !cleanNoTags.includes('}') && !cleanNoTags.includes(';')) {
      return true;
    }
    if (s.endsWith('.') || s.endsWith(':') || s.endsWith('?') || s.endsWith('!')) {
      const words = cleanNoTags.split(/\s+/);
      if (words.length >= 3 && !/[{}=+\*\/]/.test(cleanNoTags)) {
        return true;
      }
    }
    return false;
  ...
  function formatSectionContent(rawContent, subjectSlug) {
    ...
    const text = BoilerplateCleaner.cleanText(rawContent);
    if (!text) return '';
    ...
    while (i < lines.length) {
      ...
      // 1. Check for Example / Syntax Header
      const exampleType = isExampleHeader(line);
      if (exampleType === 'explained') {
      ...
      if (exampleType === 'example') {
        ...
        while (i < lines.length) {
          ...
          if (isDefiniteCodeLine(nextL) || isCssSelectorBeforeBrace(lines, i) || isExampleHeader(nextL)) {
            break;
          }

          if (isProseLine(nextL) || /^"[^"]+":?$/.test(nextL) || introDescLines.length > 0) {
            introDescLines.push(nextL);
            i++;
            if (nextL.endsWith(':') || nextL.endsWith('.')) {
              break;
            }
          } else {
            break;
        ...
        if (introDescLines.length > 0) {
          output.push(`<p>${formatInlineProse(introDescLines.join(' '))}</p>`);
        }
        ...
        while (i < lines.length) {
          ...
          if (isExampleHeader(trimmedCl)) break;
          if (/^(Note|Important|Tip|Warning):\s*/i.test(trimmedCl)) break;
          if (/^Start\s+tag$/i.test(trimmedCl)) break;
          if (isTableHeaderPair(trimmedCl, (lines[i + 1] || '').trim())) break;

          if (codeLines.length >= 1 && openBraces > 0 && openBraces === closedBraces && isProseLine(trimmedCl)) {
            break;
          }
          if (codeLines.length >= 1 && /<\/[a-zA-Z0-9\-]+>$/.test(codeLines[codeLines.length - 1]) && isProseLine(trimmedCl)) {
            break;
          }

          const opens = (trimmedCl.match(/\{/g) || []).length;
          const closes = (trimmedCl.match(/\}/g) || []).length;
          openBraces += opens;
          closedBraces += closes;
        ...
        if (cleanCode) {
          output.push(renderCodeCard(cleanCode, subjectSlug, headerLabel.toUpperCase()));
        }
        continue;
      }

      // 2. Note / Tip / Warning Callout
      if (/^(Note|Important|Tip|Warning):\s*/i.test(line)) {
        ...
        i++;
        while (i < lines.length && lines[i].trim() && !/^(Note|Important|Tip|Warning|Example):/i.test(lines[i].trim()) && !isDefiniteCodeLine(lines[i].trim()) && !isCssSelectorBeforeBrace(lines, i)) {
          body += ' ' + lines[i].trim();
          i++;
        }
        output.push(`
      ...
      if (isDefiniteCodeLine(line) || isCssSelectorBeforeBrace(lines, i)) {
        const codeLines = [];
        let openBraces = 0;
        let closedBraces = 0;
        ...
        while (i < lines.length) {
          ...
          if (openBraces > 0 && openBraces === closedBraces && isProseLine(trimmedCl)) {
            break;
          }
          if (/<\/[a-zA-Z0-9\-]+>$/.test(codeLines[codeLines.length - 1] || '') && isProseLine(trimmedCl)) {
            break;
          }
          ...
          if (/^<\/(html|svg|xml|body)>$/i.test(trimmedCl)) {
            break;
          }
          ...
          if (openBraces > 0 && openBraces === closedBraces) {
            ...
            if (nextIdx < lines.length) {
              ...
              if (isProseLine(nextL) || isExampleHeader(nextL) || isCssSelectorBeforeBrace(lines, nextIdx)) {
                break;
              }
            }
          }
      ...
      while (i < lines.length) {
        ...
        if (isExampleHeader(nextLine) || isDefiniteCodeLine(nextLine) || isCssSelectorBeforeBrace(lines, i) || /^(Note|Important|Tip|Warning):\s*/i.test(nextLine) || isPotentialKey(nextLine) || /^Start\s+tag$/i.test(nextLine)) {
          break;
        }
  ...
  async function renderTopicView(rawSubjectSlug, rawTopicId) {
    ...
    try {
      ...
      // Render Dedicated Code Examples (only examples not already rendered in sections)
      let codeExamplesHtml = '';
      if (Array.isArray(topic.code_examples) && topic.code_examples.length > 0) {
        const renderedTextNoSpace = sectionsHtml.replace(/\s+/g, '');
        topic.code_examples.forEach((ex, exIdx) => {
          const exClean = (ex.code || '').replace(/\s+/g, '');
          if (exClean && renderedTextNoSpace.includes(exClean)) {
            return; // Skip duplicate snippet already presented in sections
          }
