...
const elements = {
    ...
    btnPasteMd: document.getElementById('btn-paste-md'),
    btnEditMd: document.getElementById('btn-edit-md'),
    btnToggleWidth: document.getElementById('btn-toggle-width'),
    btnWelcomePaste: document.getElementById('btn-welcome-paste'),
    ...
    pasteModalTitleText: document.getElementById('paste-modal-title-text'),
    ...
    btnClipboardPaste: document.getElementById('btn-clipboard-paste'),
    ...
    if (state.tabs.length === 0) {
        ...
        elements.viewerContent.innerHTML = `
            <div class="welcome-screen">
                ...
                <p>Drag and drop a Markdown file here, click Open File, or paste raw Markdown text directly.</p>
                <div class="welcome-actions">
                    ...
                    <button class="secondary-btn" id="welcome-btn-paste-inner"><span class="material-icons-round">content_paste</span> Paste Markdown</button>
                </div>
            </div>
        `;
        const innerBtn = document.getElementById('welcome-btn-paste-inner');
        if (innerBtn) {
            innerBtn.addEventListener('click', () => openPasteModal(false));
        }
...
function setupKeyboardShortcuts() {
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape') {
            if (elements.pasteModal && !elements.pasteModal.classList.contains('hidden')) {
                closePasteModal();
            }
...
/**
 * Paste & Raw Markdown Editing Logic
 */
let isEditingCurrentTab = false;

function setupPasteEvents() {
    if (elements.btnPasteMd) {
        elements.btnPasteMd.addEventListener('click', () => openPasteModal(false));
    }
    if (elements.btnEditMd) {
        elements.btnEditMd.addEventListener('click', () => {
            if (!getActiveTab()) {
                Utils.showToast('No document open. Creating new Markdown draft.', 'info');
                openPasteModal(false);
            } else {
                openPasteModal(true);
            }
    ...
    if (elements.btnWelcomePaste) {
        elements.btnWelcomePaste.addEventListener('click', () => openPasteModal(false));
    }
    if (elements.btnToggleWidth) {
        elements.btnToggleWidth.addEventListener('click', toggleFullWidthMode);
    }

    if (elements.btnClosePasteModal) {
        elements.btnClosePasteModal.addEventListener('click', closePasteModal);
    }
    if (elements.btnCancelPaste) {
        elements.btnCancelPaste.addEventListener('click', closePasteModal);
    }
    if (elements.btnClearPaste) {
        elements.btnClearPaste.addEventListener('click', () => {
    ...
    if (elements.btnClipboardPaste) {
        elements.btnClipboardPaste.addEventListener('click', async () => {
            try {
                ...
                if (text) {
                    elements.pasteTextarea.value = text;
                    Utils.showToast('Pasted content from clipboard!', 'success');
                } else {
                    Utils.showToast('Clipboard is empty or has no plain text.', 'warning');
                }
            } catch (e) {
                Utils.showToast('Please use Ctrl+V inside text box to paste.', 'info');
                elements.pasteTextarea.focus();
            }
        });
    }

    if (elements.btnSubmitPaste) {
        elements.btnSubmitPaste.addEventListener('click', handlePasteSubmit);
    }
    ...
    window.addEventListener('paste', (e) => {
        ...
        if (pastedText && pastedText.trim().length > 0) {
            e.preventDefault();
            openPasteModal(false, pastedText);
            Utils.showToast('Pasted Markdown text captured into editor!', 'info');
        }
    });
}

function openPasteModal(isEdit = false, defaultText = '') {
    isEditingCurrentTab = isEdit;
    const activeTab = getActiveTab();

    if (isEdit && activeTab) {
        elements.pasteModalTitleText.textContent = 'Edit Document Markdown';
        elements.pasteDocName.value = activeTab.name;
        elements.pasteTextarea.value = activeTab.content;
    } else {
        elements.pasteModalTitleText.textContent = 'Paste / New Markdown';
        elements.pasteDocName.value = `Untitled-${Math.floor(Math.random() * 9000 + 1000)}.md`;
        elements.pasteTextarea.value = defaultText || '';
    }

    elements.pasteModal.classList.remove('hidden');
    setTimeout(() => {
        if (!defaultText && !isEdit) {
            elements.pasteTextarea.focus();
        }
    }, 100);
}
...
function handlePasteSubmit() {
    const docName = elements.pasteDocName.value.trim() || 'Untitled.md';
    const content = elements.pasteTextarea.value;

    if (!content.trim()) {
        Utils.showToast('Please enter or paste Markdown text.', 'warning');
        return;
    }
    ...
    if (isEditingCurrentTab) {
        const activeTab = getActiveTab();
        if (activeTab) {
            activeTab.name = docName;
            activeTab.content = content;
            activeTab.size = content.length;
        ...
        setActiveTab(tabId);
        renderRecentFilesList();
        Utils.showToast('Markdown document rendered!', 'success');
    }
...
function toggleFullWidthMode() {
    const isFullWidth = elements.viewerContent.classList.toggle('full-width-mode');
    elements.btnToggleWidth.classList.toggle('active', isFullWidth);
    Utils.showToast(isFullWidth ? 'Full Width View Mode enabled' : 'Centered Column View Mode enabled', 'info');
}
...
function togglePresentationMode() {
    ...
    state.presentationMode.fontScale = 1.0;
    
    // Split raw markdown content by horizontal rules (---) or major headings if single slide
    const rawContent = activeTab.content;
    let rawSlides = rawContent.split(/^\s*---\s*$/gm).map(s => s.trim()).filter(s => s.length > 0);
    
    // If no --- rules and text is long, split by H1/H2 headings for cleaner presentation slides
    if (rawSlides.length <= 1 && rawContent.length > 400) {
        const headingSplit = rawContent.split(/(?=^#{1,2}\s+)/gm).map(s => s.trim()).filter(s => s.length > 0);
        if (headingSplit.length > 1) {
            rawSlides = headingSplit;
        }
    }

    if (rawSlides.length === 0) {
        rawSlides = [activeTab.content];
    }
    
    state.presentationMode.slides = rawSlides;
    ...
    presContainer.innerHTML = `
        <div class="presentation-header search-exclude">
            ...
            <div class="presentation-actions">
                <button class="pres-btn" id="btn-pres-font-down" title="Decrease Font Size"><span class="material-icons-round">remove</span></button>
                <button class="pres-btn" id="btn-pres-font-up" title="Increase Font Size"><span class="material-icons-round">add</span></button>
    ...
    document.getElementById('btn-pres-font-up').addEventListener('click', () => {
        state.presentationMode.fontScale = Math.min(2.5, (state.presentationMode.fontScale || 1.0) + 0.15);
        renderPresentationSlide();
    });

    document.getElementById('btn-pres-font-down').addEventListener('click', () => {
        state.presentationMode.fontScale = Math.max(0.6, (state.presentationMode.fontScale || 1.0) - 0.15);
        renderPresentationSlide();
    });
...
async function renderPresentationSlide() {
    ...
    // Apply smart presentation font scaling based on content length & user scale
    const textLength = slideMarkdown.length;
    const scale = state.presentationMode.fontScale || 1.0;
    ...
    slideDiv.querySelectorAll('h1').forEach(el => el.style.fontSize = `${(h1Rem * scale).toFixed(2)}rem`);
    slideDiv.querySelectorAll('h2').forEach(el => el.style.fontSize = `${(h2Rem * scale).toFixed(2)}rem`);
    slideDiv.querySelectorAll('h3').forEach(el => el.style.fontSize = `${(h2Rem * 0.85 * scale).toFixed(2)}rem`);
    slideDiv.querySelectorAll('p, li, blockquote').forEach(el => el.style.fontSize = `${(baseRem * scale).toFixed(2)}rem`);
    slideDiv.querySelectorAll('pre, code').forEach(el => el.style.fontSize = `${(baseRem * 0.85 * scale).toFixed(2)}rem`);

    const indicator = document.querySelector('.slide-indicator');
    if (indicator) {
        indicator.textContent = `Slide ${idx + 1} of ${state.presentationMode.slides.length}`;
    }
